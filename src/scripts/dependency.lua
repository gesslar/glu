...
local DependencyQueue = Glu.registerClass({
  class_name = "DependencyQueueClass",
  script_name = script_name,
  parent_class = Glu.queue,
  dependencies = deps,
})

function DependencyQueue.setup(___, self, packages, cb)
...
function mod.setup(___, self)
  ...
  function self.new(packages, cb)
    local queue = DependencyQueue.new(packages, cb)
